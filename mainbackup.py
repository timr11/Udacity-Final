...
class WikiPage(Handler):
	def get(self, url):
		...
		if logged_in and not content:
			...
			self.render('apage_form.html', content=content, edit_url="/_edit%s" % url,
				logged_in=logged_in, username=user)
...
class EditPage(Handler):
	def get(self, url):
		if logged_in:
			...
			if content:
				self.render("editpage.html", content=content, edit_url=edit_url, 
					logged_in=logged_in, username=user)
			else:
...
app = webapp2.WSGIApplication([('/signup', Registration), 
							   ('/login', Login),
							   ('/logout', Logout), 
